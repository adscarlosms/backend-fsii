...
export default class ReservaDAO {
    async gravar(reserva) {
        if (reserva instanceof Reserva) {
            ...
            try {
                const sql = "INSERT INTO reserva(codigo,data_inicio,data_fim) VALUES(?,str_to_date(?,'%d/%m/%Y'),str_to_date(?,'%d/%m/%Y'))";
                const parametros = [reserva.cliente.codigo, reserva.data_inicio, reserva.data_fim];
                const retorno = await conexao.execute(sql, parametros);
                reserva.codigo = retorno[0].insertId;


                const sql2 = "INSERT INTO reserva_cliente(reserva_codigo,cliente_codigo,quarto_codigo) VALUES(?,?,?)";
                for (const item of reserva.itens) {
                    const parametros2 = [reserva.reserva_codigo, item.cliente_codigo, item.quarto_codigo];
    ...
    async consultar(termoBusca) {
        ...
        if (!isNaN(termoBusca)) {
            const conexao = await conectar();
            const sql = ` SELECT r.codigo, r.data_inicio, r.data_fim, q.idquarto,
                            q.numero, q.andar, q.status, q.idtpquarto, c.nome, c.endereco, 
                            c.telefone, t.idtpquarto, t.descricao,rc.reserva_codigo,
                            rc.cliente_codigo,rc.quarto_codigo 
                          FROM reserva as r 
                            INNER JOIN cliente as c 
                            ON r.cliente_codigo = c.codigo 
                            INNER JOIN reserva_cliente 
                            as rc ON rc.reserva_codigo = r.codigo 
